...
if (!(window as any).DX) (window as any).DX = {};
(window as any).DX.style = {
  instance: null,
  create: () => document.createElement("dx-style"),
  read: () => (window as any).DX.style.instance,
  update: (props: any) => {
    for (const key in props) {
      (window as any).DX.style.instance.setAttribute(key, props[key]);
    }
  },
  delete: () => (window as any).DX.style.instance.remove(),
  docs: () => {
...
class DomxStyle extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.renderAll();
    (window as any).DX.style.instance = this;
  }
  attachStyleSheet() {
    document.adoptedStyleSheets = [this.styleSheet, ...document.adoptedStyleSheets];
  }
  generateStyle(selector: string, rules: string) {
  ...
  startObserver() {
    new MutationObserver((mutationsList) => {
      for (const mutation of mutationsList) {
        if (mutation.type === "attributes") this.renderAll();
      }
